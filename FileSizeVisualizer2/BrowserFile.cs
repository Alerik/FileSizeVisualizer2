...
namespace FileSizeVisualizer2
{
	public class BrowserFile
	{
		...
		public List<BrowserFile> Folders => Children.Select(f => f).Where(f => f.FileType == FileTypes.Folder).ToList();
		public List<BrowserFile> Files => Children.Select(f => f).Where(f => f.FileType == FileTypes.File).ToList();
